...
def _llm_markdown_openai_compatible(chunk: str, title: str | None) -> str:
    ...
    if not api_key:
        raise RuntimeError("Missing GROQ_API_KEY for notes generation")
    
    # Force Groq API configuration
    base_url = "https://api.groq.com/openai/v1"
    client = OpenAI(api_key=api_key, base_url=base_url)

    # Force Llama 3.1 8B Instant model
...
def iter_generate_notes_from_texts(
    ...
    for i in range(0, n, max(1, group_size)):
        ...
        # Retry with exponential backoff to avoid 429 limits
        attempt = 0
        md = ""
        while attempt <= max_retries:
            try:
                ...
                wait_time = min(pause_seconds * (2 ** (attempt - 1)) + (attempt * 0.5), 30.0)
                time.sleep(wait_time)
